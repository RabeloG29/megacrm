...
export function DealDrawer({ deal, stages, pipelines, isAdmin, onClose, onStageChange, onChanged }: DealDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end" role="dialog" aria-label="Detalhe do lead">
      ...
      <aside
        className="relative z-10 flex h-full w-full max-w-full flex-col overflow-y-auto border-l border-[rgba(22,163,74,0.25)] bg-[#F3FBF6] shadow-[0_0_60px_rgba(22,163,74,0.12)] transition-transform duration-[400ms] ease-[cubic-bezier(0.4,0,0.2,1)] sm:w-[460px]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="sticky top-0 z-10 flex items-start justify-between gap-3 border-b border-[rgba(22,163,74,0.12)] bg-[#F3FBF6]/95 px-5 py-4 backdrop-blur">
          ...
          <button onClick={onClose} aria-label="Fechar" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-[var(--color-text-secondary)] transition hover:bg-[rgba(22,163,74,0.06)] hover:text-[var(--color-text-primary)]">
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          <div className="space-y-6 p-5">
            ...
            <section className="space-y-2">
              ...
              {deal.status === 'open' ? (
                ...
                <div className="space-y-2 rounded-lg border border-[rgba(22,163,74,0.12)] bg-[rgba(22,163,74,0.05)] p-3">
            ...
            <section className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={submitNote} disabled={savingNote || !note.trim()} className="rounded-lg bg-gradient-to-br from-[#14532D] to-[#16A34A] px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-60">
                  {savingNote ? '…' : 'Anotar'}
                </button>
              </div>
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-lg border border-[rgba(22,163,74,0.25)] bg-[#F3FBF6] p-1 shadow-[0_0_30px_rgba(22,163,74,0.15)]">
            {catalog.length === 0 ? (
              ...
              catalog.map((p) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-md px-3 py-1.5 text-left text-sm text-[var(--color-text-primary)] hover:bg-[rgba(22,163,74,0.06)]"
                  >
                    <span
  ...
  return (
    <div>
      <input
        ...
        className="w-full rounded border border-[rgba(22,163,74,0.3)] bg-[rgba(22,163,74,0.08)] px-2 py-1 text-sm text-[var(--color-text-primary)] outline-none focus:border-[var(--accent-primary)]"
      />
    ...
    <section className="space-y-2">
      ...
      <div className="relative">
        ...
        {open && (q.trim() || suggestions.length > 0) && (
          <>
            <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} />
            <div className="absolute z-20 mt-1 w-full rounded-lg border border-[rgba(22,163,74,0.25)] bg-[#F3FBF6] p-1 shadow-[0_0_30px_rgba(22,163,74,0.15)]">
              {suggestions.map((s) => (
                <button key={s.id} onClick={() => void add(s.name)} className="flex w-full items-center gap-2 rounded-md px-3 py-1.5 text-left text-sm text-[var(--color-text-primary)] hover:bg-[rgba(22,163,74,0.06)]">
                  {s.name}
                </button>
              ))}
              {q.trim() && !exact && (
                <button onClick={() => void add(q.trim())} className="flex w-full items-center gap-2 rounded-md px-3 py-1.5 text-left text-sm text-[var(--accent-secondary)] hover:bg-[rgba(22,163,74,0.06)]">
    ...
    <div>
      ...
      {field.field_type === 'boolean' ? (
        <label className="inline-flex cursor-pointer items-center gap-2 text-sm text-[var(--color-text-primary)]">
          <input type="checkbox" checked={local === 'true'} onChange={(e) => { const v = e.target.checked ? 'true' : 'false'; setLocal(v); commit(v); }} className="h-4 w-4 accent-[#16A34A]" />
          {local === 'true' ? 'Sim' : 'Não'}
        </label>
...
function CustomFieldModal({ onClose, onCreate }: {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-2xl border border-[rgba(22,163,74,0.25)] bg-[#F3FBF6] p-5 shadow-[0_0_40px_rgba(22,163,74,0.15)]">
        ...
        <div className="space-y-3">
          ...
          <label className="flex cursor-pointer items-center gap-2 text-sm text-[var(--color-text-primary)]">
            <input type="checkbox" checked={required} onChange={(e) => setRequired(e.target.checked)} className="h-4 w-4 accent-[#16A34A]" /> Obrigatório
          </label>
          {err && <div className="text-xs text-[#EF4444]">{err}</div>}
        </div>
        <div className="mt-5 flex gap-2">
          <button onClick={submit} disabled={busy} className="flex-1 inline-flex items-center justify-center gap-1 rounded-lg bg-gradient-to-br from-[#14532D] to-[#16A34A] px-3 py-2 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-60">
            <Check className="h-4 w-4" /> {busy ? 'Criando…' : 'Criar campo'}
          </button>
          <button onClick={onClose} className="rounded-lg border border-[rgba(22,163,74,0.2)] px-3 py-2 text-sm text-[var(--color-text-secondary)]">Cancelar</button>
        </div>
      </div>
    </div>
  );
