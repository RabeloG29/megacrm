...
const STAGE_COLORS = ['#16A34A', '#4ADE80', '#10B981', '#FBBF24', '#F87171', '#A78BFA', '#5B7566'];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onClick={onClose}>
      <div
        ...
        className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-[rgba(22,163,74,0.25)] bg-[#F3FBF6] shadow-[0_0_40px_rgba(22,163,74,0.15)]"
      >
        <div className="flex items-center justify-between border-b border-[rgba(22,163,74,0.12)] px-5 py-4">
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-2">
              {pipelines.map((p) => (
                <div
                  key={p.id}
                  className={`rounded-lg border px-3 py-2 ${p.id === selectedId ? 'border-[var(--accent-primary)] bg-[rgba(22,163,74,0.06)]' : 'border-[rgba(22,163,74,0.15)]'}`}
                >
                  ...
                  <input
                    ...
                    className="mt-1 w-full rounded border border-[rgba(22,163,74,0.15)] bg-transparent px-2 py-1 text-xs text-[var(--color-text-secondary)] outline-none focus:border-[var(--accent-primary)]"
                  />
                </div>
              ))}
            </div>
            <div className="flex gap-2">
              ...
              <button onClick={handleCreateFunil} disabled={busy || !newFunil.trim()} className="rounded-lg bg-gradient-to-br from-[#14532D] to-[#16A34A] px-3 py-2 text-sm font-semibold text-white disabled:opacity-60">
          ...
          <section className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="rounded-lg bg-gradient-to-br from-[#14532D] to-[#16A34A] px-3 py-2 text-sm font-semibold text-white disabled:opacity-60"
              >
                <Plus className="h-4 w-4" />
              </button>
            </div>
...
function StageRow({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className="rounded-lg border border-[rgba(22,163,74,0.15)] bg-[rgba(22,163,74,0.05)] px-2 py-2"
    >
      <div className="flex items-center gap-2">
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1.5 pl-6">
        <div className="flex items-center gap-1">
          {STAGE_COLORS.map((c) => (
            <button
              ...
              className={`h-3.5 w-3.5 rounded-full ring-offset-1 ring-offset-[#F3FBF6] ${stage.color === c ? 'ring-2 ring-white' : ''}`}
              style={{ background: c }}
              aria-label={`Cor ${c}`}
            />
          ))}
        ...
        <label className="flex items-center gap-1 text-[10px] uppercase tracking-wide text-[var(--color-text-secondary)]">
          ...
          <input
            ...
            className="w-14 rounded border border-[rgba(22,163,74,0.2)] bg-[rgba(22,163,74,0.06)] px-1.5 py-0.5 text-xs text-[var(--color-text-primary)] outline-none focus:border-[var(--accent-primary)]"
          />
          %
        </label>
      </div>
      ...
      <div className="mt-1.5 pl-6">
        <textarea
          ...
          className="w-full resize-y rounded border border-[rgba(22,163,74,0.15)] bg-[rgba(22,163,74,0.05)] px-2 py-1 text-[11px] text-[var(--color-text-primary)] outline-none placeholder:text-[var(--color-text-secondary)] focus:border-[var(--accent-primary)]"
        />
      </div>
      ...
      {confirming && (
        <div className="mt-2 space-y-2 rounded-md border border-[rgba(239,68,68,0.25)] bg-[rgba(239,68,68,0.06)] p-2">
          ...
          <div className="flex items-center gap-2">
            <select value={moveTo} onChange={(e) => setMoveTo(e.target.value)} className="flex-1 rounded border border-[rgba(22,163,74,0.2)] bg-[rgba(22,163,74,0.06)] px-2 py-1 text-xs text-[var(--color-text-primary)] outline-none">
