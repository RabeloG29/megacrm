  ...
  return (
    <Dialog open={open} onClose={onClose} title="Reiniciar com template" widthClass="max-w-lg">
      <div className="space-y-4">
        ...
        {selected && (
          <div className="rounded-lg border border-[rgba(22,163,74,0.12)] bg-[rgba(22,163,74,0.05)] p-3 text-sm text-[var(--color-text-secondary)] whitespace-pre-wrap">
            {selected.body}
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            {vars.map((idx) => (
              <div key={idx} className="flex items-center gap-2">
                <span className="font-mono text-xs bg-[rgba(22,163,74,0.06)] rounded px-2 py-1 text-[var(--accent-primary)]">
                  {`{{${idx}}}`}
                </span>
                <Input
                  value={values[idx] ?? ''}
