      ...
      <div className="flex flex-wrap gap-1.5">
        ...
        <div className="relative">
          <button
            ...
            className="inline-flex items-center gap-1 rounded-full border border-[rgba(22,163,74,0.25)] px-2.5 py-1 text-[11px] font-semibold text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]"
          >
            <Plus className="h-3 w-3" />
            Tag
          </button>
          {adding && (
            <div className="absolute z-20 mt-1 w-48 rounded-lg border border-[rgba(22,163,74,0.25)] bg-white shadow-2xl overflow-hidden max-h-56 overflow-y-auto">
              {available.length === 0 ? (
                ...
                available.map((t) => (
                  <button
                    ...
                    type="button"
                    onClick={() => attach(t.id)}
                    className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-[rgba(22,163,74,0.12)]"
                  >
