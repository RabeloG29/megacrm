...
export function OrgSwitcher() {
  ...
  if (!isSuperAdmin) {
    return <div className="hidden md:block pr-2 border-r border-[rgba(22,163,74,0.1)]">{label}</div>;
  }

  return (
    <div ref={rootRef} className="relative hidden md:block pr-2 border-r border-[rgba(22,163,74,0.1)]">
      <button
        onClick={() => setOpen((v) => !v)}
        aria-label="Trocar de organização"
        className="flex items-center gap-1.5 rounded-lg px-2 py-1 -my-1 transition hover:bg-[rgba(22,163,74,0.06)]"
      >
        {label}
        <ChevronDown
      ...
      {open ? (
        <div className="absolute right-2 top-[calc(100%+10px)] z-50 w-64 overflow-hidden rounded-2xl border border-[rgba(22,163,74,0.25)] bg-white p-1.5 shadow-[0_10px_40px_rgba(0,0,0,0.15)]">
          ...
          {orgs === null ? (
            ...
            orgs.map((org) => (
              <button
                ...
                className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-sm text-[var(--color-text-primary)] transition hover:bg-[rgba(22,163,74,0.06)] disabled:cursor-not-allowed disabled:opacity-50"
              >
